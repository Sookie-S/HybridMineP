...
print ($output "Sequence,Best Hit,e-value,Identities,Positives,Gaps\n");

my $i = 0;
my $j = 0;

print ("Input: $ARGV[0]\n");
open( my $input, "<", $ARGV[0]) or die $!; # file containing blast results
#print ("Input: $input\n");
...
while(<$input>) {
	...
	if ( $newHit == 1 and ( $_ =~ m/^(\S+) .*\d+ +\d+.\d+ +$/ or $_ =~ m/^(\S+) .*\d+ +\d+e\-\d+$/) ) {
		...
		if ( $1 ne $geneq) {
			...
			if ( $_ =~ m/\S+\s+\S+\s+(\d+e\-\d+)\s*$/ or $_ =~ m/\S+\s+\S+\s+(\d*\.\d*)\s*$/) {
				...
				print ("query : $geneq - hit : $hit - evalue : $evalue ($1)");
				#print ("$_\n");
			}	
	...
	if ( $newID == 1 and ($_ =~ m/\s*Identities = \d+\/\d+ \(\w+\%\), Positives = \d+\/\d+ \(\w+\%\), Gaps = \d+\/\d+ \(\w+\%\)$/)) {
		# print ($_);


		$idLine = $_;
		$identities = $1 if ($_ =~ /Identities = \d+\/\d+ \((\w+)\%\)/);
		$positives = $1 if ($_ =~ /Positives = \d+\/\d+ \((\w+)\%\)/);
		$gaps = $1 if ($_ =~ /Gaps = \d+\/\d+ \((\w+)\%\)/);

		print (" $idLine\n");

		print ($output "$ARGV[1].$geneq,$ARGV[2].$hit_backup,$evalue_backup,$identities,$positives,$gaps,$idLine" );
